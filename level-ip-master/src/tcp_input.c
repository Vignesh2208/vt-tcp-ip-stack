...
uint8_t * allock_new_data_block(int size) {
...
int merge_skbs(struct vsock *sk, struct sk_buff * prev_skb, struct sk_buff * skb) {
    uint8_t * data_block;
    ...
    data_block = allock_new_data_block(hdr_space + skb->dlen + prev_skb->dlen);
    if (!data_block)
        return -1;
    ...
    skb_reserve(prev_skb, hdr_space + prev_skb->dlen);
...
static int tcp_clean_rto_queue(struct vsock *sk, uint32_t una) {
    ...
    if (tsk->chk_pt_send_queue_size > 0) {
        ...
        list_for_each_safe_from(item, tmp, &chk_pt_resume_skb->list, &sk->write_queue.head) {
            ...
            if (th->psh) {
                if (prev_skb && skb->dlen + prev_skb->dlen <= tsk->smss) {
                    assert(prev_skb->dlen > 0);
                    
                    rc = merge_skbs(sk, prev_skb, skb);
                    if (rc < 0)
                        break;
    ...
    if (prev_skb && rc == 0) {
        // this is last skb in queue. set psh flag for this.
        tmpth = tcp_hdr(prev_skb);
        tmpth->ack = 1;
        tmpth->psh = 1;
...
static struct tcp_sock *tcp_listen_child_sock(struct tcp_sock *tsk,
    ...
    struct vsocket * newvsocket = alloc_socket();
	if (!newvsocket) {
        free(newsk);
        return NULL;
    }
...
int tcp_input_state(struct vsock *sk, struct tcphdr *th, struct sk_buff *skb) {
    ...
    switch (sk->state) {
        ...
        case TCP_FIN_WAIT_2:
            if (th->psh || skb->dlen > 0) {
                if (tcp_data_queue(tsk, th, skb) < 0) {
    ...
    case TCP_FIN_WAIT_2:
        if (expected) {
            ...
            int pending = skb_queue_len(&sk->write_queue) > 0;
            ...
            } else if (th->psh || (++tsk->delacks > 1)) {
            //} else if ((tsk->chk_pt_rcv_queue_size > 1000 && ++tsk->delacks > 1)) {
                tsk->delacks = 0;
                ...
                //if (skb->dlen < 1000)
                //    printf ("Adding del-ack for small pkt. Dlen = %d, delacks = %d\n", skb->dlen, tsk->delacks);
...
int tcp_receive(struct tcp_sock *tsk, void *buf, int len) {
    ...
    if (rlen >= 0 && tsk->sk.err >= 0) tcp_rearm_user_timeout(sk);
